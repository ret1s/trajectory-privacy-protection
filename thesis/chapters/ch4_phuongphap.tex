...
\]
Khi đó với \emph{mọi} biến cố đo được $S$,
\[
\frac{K_A(S\mid x)}{K_A(S\mid x')}
=\frac{K(S\cap A\mid x)}{K(S\cap A\mid x')}\cdot\frac{K(A\mid x')}{K(A\mid x)}
\le e^{\eps \dEu(x,x')}\cdot e^{\eps \dEu(x,x')} = e^{2\eps \dEu(x,x')}.
\]
Cả hai giả thiết đều \emph{bắt buộc}: bỏ giả thiết $\eps$-Geo-I (ví dụ
...
\label{thm:smrem}
\emph{Giả thiết khởi tạo:} cache rỗng cho ô $C$ ở đầu scope (hoặc một trạng thái
cache/reset công khai chung). Cố định một vị trí thật $x$ thuộc ô $C$ và cho
người dùng báo cáo $x$ đúng $T$ lần. Gọi $Z \sim \mathrm{REM}(\mathrm{rep}(C))$
là release đầu (cache miss đầu tiên). Transcript
