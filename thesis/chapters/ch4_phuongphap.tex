...
Với $z=h$ và $z\ne h$, viết tường minh hai nhánh của $K_x$; tỷ số mỗi nhánh bị
chặn bởi tích của test-ratio ($\le e^{\eps_{\text{test}}d}$, query độ nhạy 1 +
Laplace) và REM-ratio ($\le e^{\eps_{\text{release}}d}$, Định lý~\ref{thm:rem}),
cho $e^{(\eps_{\text{test}}+\eps_{\text{release}})d}$. Kiểm chứng bằng liệt kê
hai-vertex: $a{=}0$m, $b{=}100$m, $h{=}a$, $\theta{=}200$m,
$\eps_{\text{test}}{=}\eps_{\text{release}}{=}0{,}01$; với $z{=}b$ tỷ số
$=e^{1.5}$, vượt chặn cũ $e^{1}$ nhưng thỏa chặn đúng $e^{2}$.
\end{proof}
