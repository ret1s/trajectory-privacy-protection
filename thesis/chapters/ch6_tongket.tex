...
scalar $\eps_w$ w-event cần budget manager theo cửa sổ (chưa cài). SM-REM chỉ đảm
bảo vị trí tĩnh lặp lại, pattern revisit rò rỉ.

\textbf{Phạm vi \& provenance.} Artifact là reference mechanism + simulator Flask
phía server, chưa phải hiện thực on-device; cache/định danh lifecycle (R8--R12)
ngoài phạm vi. Averaging đa-home dùng stay-point (không phải nhãn cư trú), moving
benchmark còn một-seed; clean-clone provenance (build script, seed dẫn xuất, raw
rows, dependency lock) đang bổ sung. Chưa chạy RAoPT; chưa so GEM (metric đường
đi ngắn nhất)/PIM. Chi tiết đối chiếu ở \texttt{docs/reviews/}.
