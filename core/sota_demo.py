"""Dependency-light, paper-inspired comparators for the SOTA demo.

These classes are deliberately named ``*Lite`` because they are **not faithful
reproductions** of the cited systems.  They provide runnable representatives of
three different dummy-generation output contracts while the full benchmark-v2
adapters, learned models, paper datasets, and mechanism-aware attackers are
still being built:
...
None of these heuristics has a Geo-Indistinguishability, differential-privacy,
or k-anonymity theorem.  Results must be labelled ``DEMO / PAPER-INSPIRED`` and
must not be reported as reproductions of the source papers.  All geographic
computation is local: no map or routing web API is called.
"""
...
class CandidateSetRelease:
    """One public candidate set plus evaluator-only truth for the demo.
    ...
    def public_candidates(self) -> Tuple[LatLon, ...]:
        """The only portion safe to expose to the demo attacker/LSP."""
...
def _network_distances(
    ...
    disconnected extracts may not; falling back keeps the demo runnable while
    ...
    # reachable one, but retain a finite score so the smoke demo never crashes.
...
class TransProtectLite:
    """Paper-inspired, non-learned TransProtect demo comparator.
    ...
    This lite version only demonstrates the *idea* of filtering candidates by
    ...
    def protect_run(
        ...
        return make_replacement_run(
            ...
            real,
            replacement,
            public_parameters={"demo_only": True, "source_method": self.source_method},
        )
...
class SemanticDummyLite:
    """Paper-inspired temporal/semantic real-plus-dummies demo comparator.
