...
OUT = BASE/'iteration25_capped_profile.json'
...
def main():
    ...
    rows = []
    factories = {'linear_paced': partial(PacedSlackProgressLaneDummy, utility_slack=0.),
        'capped90_paced': partial(CappedServiceLaneDummy, utility_slack=0.),
        'capped90_paced_slack03': partial(CappedServiceLaneDummy, utility_slack=.03)}
    for method, cls in factories.items():
        ...
        rows.append({'method': method, 'setup_ms': setup_ms,
            ...
            'steps': len(points), 'step_ms': model.step_ms, 'mean_ms': float(np.mean(model.step_ms)),
