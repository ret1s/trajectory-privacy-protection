"""Iteration 7: bounded cover-slack development ablation."""
...
OUT=ROOT/'artifacts/benchmarks/research_loop/iteration07_slack.json'
DATA=ROOT/'artifacts/datasets/research_loop_development_v1/dataset.json'
SPECS={'raw':(0,0,0),'core_H12':(12,0,0),'filter_progress':(12,0,0),'filter_slack01':(12,0,0),'filter_slack03':(12,0,0)}
...
def main():
    ...
    for name,(h,head,delay) in SPECS.items():
        ...
        if name.startswith('filter_slack'):cls=partial(FilteredSlackProgressCoverLaneDummy,utility_slack=int(name[-2:])/100)
    ...
    for f in data['families']:
        for sess in f['sessions']:
            ...
            for name,(h,head,delay) in SPECS.items():
                ...
                public={'events':events,'public_parameters':{'head_s':head,'delay_s':delay,'nominal_horizon':h,'budget_filter':name.startswith('filter'),'service_progress':'progress' in name or 'slack' in name}}
                errors={};predictions={}
    ...
    result={'schema':'whole-session-slack-development-v1','resources':metadata,'dataset_sha256':sha(DATA),
        ...
        'source_sha256':{p:sha(ROOT/p) for p in ('core/boundary_release.py','benchmark/engines/quotient_cover.py','benchmark/engines/filtered_cover.py','benchmark/engines/progress_cover.py','benchmark/engines/slack_progress.py','docs/research/predictive_filter_argument.md','experiments/contribution_stress.py')},
