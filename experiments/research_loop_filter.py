"""Iteration 4: prospective branch-budget filter with full-session controls."""
...
OUT=ROOT/'artifacts/benchmarks/research_loop/iteration04_filter.json'
DATA=ROOT/'artifacts/datasets/research_loop_development_v1/dataset.json'
SPECS={'raw':(0,0,0),'core_H12':(12,0,0),'filter_H12':(12,0,0),'filter_boundary60':(12,60,60)}
...
def main():
    ...
    for name,(h,head,delay) in SPECS.items():
        cls=FilteredCoverLaneDummy if name.startswith('filter') else QuotientCoverLaneDummy
    ...
    for f in data['families']:
        for sess in f['sessions']:
            ...
            for name,(h,head,delay) in SPECS.items():
                ...
                public={'events':events,'public_parameters':{'head_s':head,'delay_s':delay,'nominal_horizon':h,'budget_filter':name.startswith('filter')}}
                errors={};predictions={}
    ...
    result={'schema':'whole-session-filter-development-v1','resources':metadata,'dataset_sha256':sha(DATA),
        ...
        'source_sha256':{p:sha(ROOT/p) for p in ('core/boundary_release.py','benchmark/engines/quotient_cover.py','benchmark/engines/filtered_cover.py','docs/research/predictive_filter_argument.md','experiments/contribution_stress.py')},
