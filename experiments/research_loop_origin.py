"""Iteration 12: guarded origin allocation, exposed development only."""
...
OUT=ROOT/'artifacts/benchmarks/research_loop/iteration12_origin_guard.json'
DATA=ROOT/'artifacts/datasets/research_loop_development_v1/dataset.json'
SPECS={'raw':(0,0,0),'core_H12':(12,0,0),'filter_progress':(12,0,0),'filter_origin_first':(12,0,0),'filter_origin_60':(12,0,0)}
...
def main():
    ...
    metadata={**metadata,'early_belief_sha256':early.sha256}
    ...
    for name,(h,head,delay) in SPECS.items():
        ...
        if name=='filter_origin_first':cls=partial(OriginGuardProgressLaneDummy,early_belief_model=early,guard_seconds=0.)
        if name=='filter_origin_60':cls=partial(OriginGuardProgressLaneDummy,early_belief_model=early,guard_seconds=60.)
        ...
        for sess in f['sessions']:
            ...
            for name,(h,head,delay) in SPECS.items():
                ...
                row={'family_id':f['family_id'],'split':f['split'],'session_id':sess['session_id'],
                    ...
                    'budget_bound':model.spent_bound if h else None,'quarter_units': 'origin' in name,'guard_seconds': 60. if name=='filter_origin_60' else 0. if name=='filter_origin_first' else None,'privacy_reads':sum(v['private_read'] for v in model.evaluator_ledger) if name.startswith('filter') else min(h,stream.generated) if h else None,
    ...
    result={'schema':'whole-session-origin-guard-development-v1','resources':metadata,'dataset_sha256':sha(DATA),
