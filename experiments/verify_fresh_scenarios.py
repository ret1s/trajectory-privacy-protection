"""Fresh-family native audit; base physical/scenario checks retained from v3."""
import argparse
...
DEFAULT=ROOT/'artifacts/datasets/urban_fresh_v2/dataset.json'
...
def verify_new_gates(data):
    ...
    assert {f['seed']:f['split'] for f in data['families']}=={
        s: 'fresh_validation' if s <= 306 else 'fresh_confirmation' for s in range(301,313)}
...
def audit(path=DEFAULT):
    d = read(path)
    validate_bundle(d)
    result = verify(path, raw=True)
    network = _load_sumolib().net.readNet(str(ROOT/d['network']['path']), withInternal=True)
    geometry = _load_sumolib().geomhelper
    largest = 0.
    for sid, trace in d['traces'].items():
        for p in trace:
            lane = network.getLane(p['lane_id'])
            expected = geometry.positionAtShapeOffset(lane.getShape(), p['lane_pos_m'])
            actual = network.convertLonLat2XY(p['lon'], p['lat'])
            error = math.dist(expected, actual)
            largest = max(largest, error)
            assert error <= 4.1, (sid, p['time_s'], 'lane position mismatch', error)
    for f in d['families']:
        assert len(f['sessions']) == 22 and len(f['simulation_runs']) == 9
        assert all(float(a.get('arrival', -1)) >= 0 for a in f['actual_vehicles'].values())
        for run in f['simulation_runs']:
            assert run['command'][run['command'].index('--lanechange.duration')+1] == '3'
            path_xml = next(ROOT/p for p in run['source_files'] if p.endswith('routes.rou.xml'))
            assert all(s.attrib['parking'] == 'false' for s in ET.parse(path_xml).getroot().iter('stop'))
    old = read(ROOT/'artifacts/datasets/urban_scenarios_v3/dataset.json')
    aux = read(ROOT/'artifacts/datasets/urban_shadow_v1/dataset.json')
    def sets(bundle, split=None):
        families = [f for f in bundle['families'] if split is None or f['split'] == split]
        sessions = [s for f in families for s in f['sessions']]
        identities = {(k, s[k]) for s in sessions for k in ('person_id', 'device_id', 'physical_vehicle_id')}
        routes = {tuple(s['route_edges']) for s in sessions}
        records = [r for r in bundle['records'] if split is None or r['split'] == split]
        windows = set()
        for r in records:
            for sid, indices in zip(r['session_ids'], r['observed_indices']):
                trace = bundle['traces'][sid]; start = trace[indices[0]]['time_s']
                windows.add(tuple((trace[i]['lat'], trace[i]['lon'], trace[i]['time_s']-start) for i in indices[:12]))
        return identities, routes, windows
    history = [a | b for a, b in zip(sets(old), sets(aux))]
    fresh = sets(d)
    validation, confirmation = sets(d, 'fresh_validation'), sets(d, 'fresh_confirmation')
    overlaps = {label: {'identity_overlap': len(a[0]&b[0]), 'exact_route_overlap': len(a[1]&b[1]),
        'exact_first12_window_overlap': len(a[2]&b[2]),
        'shared_external_edges': len({e for r in a[1] for e in r}&{e for r in b[1] for e in r})}
        for label, a, b in [('history_to_fresh', history, fresh), ('validation_to_confirmation', validation, confirmation)]}
    assert all(v['identity_overlap'] == 0 and v['exact_route_overlap'] == 0 for v in overlaps.values()), overlaps
    # Single-event POI/corridor windows may share public-road coordinates; report,
    # do not silently treat same-city geometry as disjoint geography.
    by_split = {split: dict(sorted(Counter(r['case_id'] for r in d['records'] if r['split'] == split).items()))
        for split in ('fresh_validation', 'fresh_confirmation')}
    assert all(len(counts) == 30 for counts in by_split.values())
    result.update(verified=True, dataset_content_sha256=content_hash(d), source_sha256=sha(__file__),
        max_lane_center_error_m=largest, overlaps=overlaps, by_split=by_split,
        rejected_records=len(d['rejections']), rejection_reasons=dict(Counter(r['reason'] for r in d['rejections'])),
        scope='same_city_new_families; partial_per_family_case_coverage; S4-S10_not_protection_evaluated_here')
    write(Path(path).parent/'verification.json', result)
