...
def main():
    ...
    for plan in plans:
        k = plan['k']
        # The server receives coordinates, not catalogue state identifiers.
        # Match every adaptive runner's coordinate-to-road access rule, including
        # coincident lane states and projection roundtrip tie behavior.
    ...
    result = {'schema': 'public-fixed-cover-service-audit-v1', 'resources': meta,
        ...
        'service_access_correction': {'original_artifact': 'iteration22_public_cover_state_access_v1.json',
            'original_sha256': sha(BASE/'iteration22_public_cover_state_access_v1.json'),
            'reason': 'Use server coordinate access, matching adaptive runners; v1 directly indexed internal planner states.'},
        'plans': plans, 'sources': sources, 'sessions': sessions,
