...
OUTPUT = ROOT/'artifacts/datasets/urban_fresh_v1'
SPLITS = {s: 'fresh_validation' if s <= 306 else 'fresh_confirmation' for s in range(301, 313)}


def build(output=OUTPUT, work=ROOT/'cache/fresh_scenarios_v1'):
    ...
    sources = set(old['source_sha256']) | {'experiments/build_fresh_scenarios.py',
    ...
    bundle = {'schema': SCHEMA, 'purpose': 'scenario_challenge_dataset_not_protection_evidence',
        ...
        'base_dataset_sha256': sha(ROOT/'artifacts/datasets/urban_scenarios_v3/dataset.json'),
...
if __name__ == '__main__':
    ...
    p.add_argument('--workdir', type=Path, default=ROOT/'cache/fresh_scenarios_v1')
