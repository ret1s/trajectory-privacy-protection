"""Development ablation: optimize the fixed top-ten reply for a top-five target."""
...
SOURCE = ROOT/'artifacts/benchmarks/research_loop/iteration13_paced_cases.json'
DATA = ROOT/'artifacts/datasets/research_loop_development_v1/dataset.json'
OUT = ROOT/'artifacts/benchmarks/research_loop/iteration15_response_cases.json'
CONTROLS = ('raw', 'filter_progress', 'filter_paced', 'filter_paced_origin')
...
def main():
    ...
    early = ResponseAwareAnchorModel(PublicAnchorModel(rn, context, prior, epsilon_release=.0025,
        epsilon_test=.0025, cache_path=CACHE/'belief_origin_quarter.npz'), deeper)
    factories = {'response_progress': MatchedFilteredProgressCoverLaneDummy,
                 'response_paced': PacedProgressLaneDummy,
                 'response_paced_origin': partial(PacedOriginGuardLaneDummy, early_belief_model=early, guard_seconds=0.)}
    ...
    for ordinal, control in enumerate(clocks):
        ...
        for method, model in models.items():
            ...
            executions.append({'session_id': sid, 'family_id': control['family_id'], 'split': control['split'],
                ...
                'quarter_units': method == 'response_paced_origin', 'evaluator_ledger': model.evaluator_ledger})
    ...
    result = {'schema': 'response-aware-case-development-v1',
        'scope': 'same L10 service and true top-five; objective alignment only; development; new learned attacks still required',
        'control_source_sha256': sha(SOURCE), 'dataset_sha256': sha(DATA), 'resources': metadata,
        ...
        'code_sha256': sha(Path(__file__)), 'source_sha256': {p: sha(ROOT/p) for p in (
            'benchmark/response_aware_belief.py', 'benchmark/engines/paced_guard.py', 'benchmark/engines/origin_guard.py',
