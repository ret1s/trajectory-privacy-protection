"""Recovered-map auxiliary SUMO routes for new sequential attackers."""
import argparse
...
OUTPUT = ROOT / 'artifacts/datasets/research_loop_shadow_v1'
WORK = ROOT / 'cache/research_loop_20260924/shadow_routes'
SOURCE_PATHS = ('data/shadow_routes.py', 'experiments/build_research_loop_shadow_routes.py',
                'data/scenario_suite/mobility.py', 'data/sumo_demo.py',
                'docs/research/algorithm_improvement_loop.md')
...
def build(output=OUTPUT, work=WORK):
    ...
    base_path = ROOT / 'artifacts/datasets/research_loop_development_v1/dataset.json'
    ...
    families = plans(network, seeds=range(1201,1281))
    for i,f in enumerate(families):
        f['split'] = 'auxiliary_train' if i<64 else 'auxiliary_selection'
        f['role'] = 'auxiliary_training' if i<64 else 'auxiliary_holdout'
    ...
    assert len(bundle['families']) == 80 and len(bundle['traces']) == 160
    assert len({tuple(f['departure_cell']) for f in families}) == 80
