"""Iteration 5: ledger by service-equivalent progress ablation."""
...
OUT=ROOT/'artifacts/benchmarks/research_loop/iteration05_progress.json'
DATA=ROOT/'artifacts/datasets/research_loop_development_v1/dataset.json'
SPECS={'raw':(0,0,0),'core_H12':(12,0,0),'filter_H12':(12,0,0),'progress_H12':(12,0,0),'filter_progress':(12,0,0),'filter_progress_boundary60':(12,60,60)}
...
def main():
    ...
    for f in data['families']:
        for sess in f['sessions']:
            ...
            for name,(h,head,delay) in SPECS.items():
                ...
                public={'events':events,'public_parameters':{'head_s':head,'delay_s':delay,'nominal_horizon':h,'budget_filter':name.startswith('filter'),'service_progress':'progress' in name}}
                errors={};predictions={}
    ...
    result={'schema':'whole-session-progress-development-v1','resources':metadata,'dataset_sha256':sha(DATA),
        ...
        'source_sha256':{p:sha(ROOT/p) for p in ('core/boundary_release.py','benchmark/engines/quotient_cover.py','benchmark/engines/filtered_cover.py','benchmark/engines/progress_cover.py','docs/research/predictive_filter_argument.md','experiments/contribution_stress.py')},
