"""Run actual BR and causal boundary layer on NEW full SUMO sessions."""
...
OUT=ROOT/'artifacts/benchmarks/research_loop/iteration03_boundary.json'
DATA=ROOT/'artifacts/datasets/research_loop_development_v1/dataset.json'
SPECS={'raw':(0,0,0),'core_H12':(12,0,0),'core_H64':(64,0,0),
       'head60':(64,60,0),'tail60':(64,0,60),'boundary60':(64,60,60)}
...
def main():
    ...
    for name,(h,head,delay) in SPECS.items():
        models[name]=Raw() if h==0 else QuotientCoverLaneDummy(rn,belief_model=beliefs[h],
    ...
    for f in data['families']:
        for sess in f['sessions']:
            ...
            for name,(h,head,delay) in SPECS.items():
                ...
                public={'events':events,'public_parameters':{'head_s':head,'delay_s':delay,'horizon':h}}
                errors={};predictions={}
                ...
                row={'family_id':f['family_id'],'split':f['split'],'session_id':sess['session_id'],
                    ...
                    'budget_bound':model.spent_bound if h else None,'privacy_reads':min(h,stream.generated) if h else None,
                    'after_horizon_events':max(0,stream.generated-h) if h else None,
                    'accounting':stream.evaluator_summary(),'evaluation_and_generation_ms':elapsed,
    ...
    result={'schema':'whole-session-boundary-development-v1','resources':metadata,'dataset_sha256':sha(DATA),
        ...
        'source_sha256':{p:sha(ROOT/p) for p in ('core/boundary_release.py','benchmark/engines/quotient_cover.py','experiments/contribution_stress.py')},
