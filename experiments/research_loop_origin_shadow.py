"""Mechanism-aware guarded-first-query inversion; matched new epsilon."""
...
SOURCE = ROOT/'artifacts/benchmarks/research_loop/iteration12_origin_guard.json'
SHADOW = ROOT/'artifacts/benchmarks/research_loop/iteration12_origin_shadow_training.json'
OUT = ROOT/'artifacts/benchmarks/research_loop/iteration12_origin_shadow_attack.json'
...
def main():
    ...
    if not SHADOW.exists():
        ...
        model = OriginGuardProgressLaneDummy(rn, belief_model=belief, early_belief_model=early, guard_seconds=0., budget=.24, horizon=12,
        ...
        training = {'schema': 'origin-guard-shadow-public-simulation-v1',
    ...
    records = [r for r in source['rows'] if r['method'].startswith('filter_origin')]
    ...
    result = {'schema': 'origin-guard-shadow-S9-development-v1', 'source_sha256': sha(SOURCE),
              ...
              'source_code_sha256': {p: sha(ROOT/p) for p in (
                  ...
                  'benchmark/engines/quotient_cover.py', 'benchmark/engines/origin_guard.py')},
