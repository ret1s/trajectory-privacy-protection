"""Development ablation: location-wise capped service with exact public reference grouping."""
...
ANCHORS = ROOT/'artifacts/benchmarks/research_loop/iteration20_mobility_cases.json'
DATA = ROOT/'artifacts/datasets/research_loop_development_v1/dataset.json'
OUT = ROOT/'artifacts/benchmarks/research_loop/iteration25_capped_service_cases.json'
...
def main():
    ...
    belief = ResponseAwareAnchorModel(base, deeper)
    factories = {'capped90_paced': partial(GroupedCappedServiceLaneDummy, utility_slack=0.),
                 'capped90_paced_slack03': partial(GroupedCappedServiceLaneDummy, utility_slack=.03)}
    models, setup = {}, {}
    for name, cls in factories.items():
        started = time.perf_counter()
        model = cls(rn, belief_model=belief, k=5, budget=.24, horizon=12, rng=np.random.default_rng(0))
        models[name] = model
        setup[name] = {'total_setup_ms': (time.perf_counter()-started)*1000,
                       'capped_index_build_ms': model.capped_index_build_ms,
                       'reference_utility_groups': model.capped_index.reference.shape[0],
                       'original_latent_states': model.capped_index.original_latent_count}
    anchor_evidence = json.loads(ANCHORS.read_text())
    anchor_lookup = {(r['session_id'], r['rep']): r['evaluator_anchor_sha256'] for r in anchor_evidence['executions'] if r['method'] == 'response_empirical_paced'}
    ...
    for ordinal, control in enumerate(clocks):
        ...
        for method, model in models.items():
            ...
            assert digest == anchor_lookup[sid, rep]
            executions.append({'session_id': sid, 'family_id': control['family_id'], 'split': control['split'],
                ...
                'planner_diagnostics': model.evaluator_objective, 'evaluator_states': model.evaluator_states})
    ...
    result = {'schema': 'capped-service-case-development-v1',
        'scope': 'location-wise cap .9 objective, total K5/L10, unchanged scoring; same anchors and ledger; core development; matched learned attacks not yet run',
        'control_source_sha256': sha(SOURCE), 'paired_anchor_source_sha256': sha(ANCHORS), 'belief_model_sha256': belief.sha256, 'dataset_sha256': sha(DATA), 'resources': metadata,
        ...
        'code_sha256': sha(Path(__file__)), 'source_sha256': {p: sha(ROOT/p) for p in (
            ...
            'benchmark/engines/slack_progress.py', 'benchmark/engines/capped_service.py',
            'benchmark/capped_service_objective.py', 'benchmark/grouped_capped_service.py',
        ...
        'setup_costs': setup, 'executions': executions, 'rows': rows, 'summaries': summaries(rows)}
