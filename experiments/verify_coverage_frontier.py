"""Independent inference, service, selection and replay checks of the frontier."""
import argparse
...
from experiments.run_coverage_frontier import prepare,generate,OUTPUT,METHODS,BUDGETS,DEPTHS,PARENT
...
def verify(budget,output=OUTPUT,check_only=False):
    ...
    for method in METHODS:
        ...
        z=(x-m['mean'])/m['scale']
        info=train['forests'][method];path=output/info['path'];assert sha(path)==info['sha256']
        classifiers[method]={}
    ...
    for phase,payload in phases.items():
        ...
        assert payload['records']==parent['records']+(auxiliary if phase=='training' else [])
        ...
        assert payload['selection_sha256']==(sha(output/'selection.json') if phase=='development' else None)
        paired={}; replays=set()
        ...
        for index,row in enumerate(payload['rows']):
            ...
            if budget==.24 and not aux and method in ('geometric','mean_greedy'):
                baseline=old[record['record_id'],row['replicate'],
                             'baseline' if method=='geometric' else 'uniform_cover']
                for key in ('evaluator_anchors','evaluator_states','spent_bound'): equal(row[key],baseline[key])
                equal(public['events'],baseline['public']['events'])
                if phase!='training':near(row['utility']['5']['poi_recall_at_5'],baseline['utility']['poi_recall_at_5'])
                counts['historical_parity']+=1
            if phase!='training':
        ...
        for summary in payload['summaries']:
            ...
            assert len({r['family_id'] for r in group})==summary['families']==(2 if phase=='validation' else 4)
    ...
    result={'verified':True,'budget':budget,'counts':dict(counts),
        ...
        'scope':'all metrics independently reconstructed; full/prefix replay subset; development only'}
...
if __name__=='__main__':
    ...
    args=p.parse_args();verify(args.budget,args.output,args.check_only)
