"""Development ablation: two fixed public queries and three adaptive residual queries."""
...
OUT = ROOT/'artifacts/benchmarks/research_loop/iteration23_public_backbone_cases.json'
...
def main():
    ...
    factories = {'public_backbone2_paced': partial(PublicBackboneLaneDummy, public_queries=2, utility_slack=0.),
                 'public_backbone2_paced_slack03': partial(PublicBackboneLaneDummy, public_queries=2, utility_slack=.03)}
    ...
    for ordinal, control in enumerate(clocks):
        ...
        for method, model in models.items():
            ...
            executions.append({'session_id': sid, 'family_id': control['family_id'], 'split': control['split'],
                ...
                'evaluator_anchor_sha256': digest, 'public_backbone_plan': model.fixed_plan,
        ...
        for rep in range(2):
            for method in factories:
                ...
                for slot, (sid, indices) in enumerate(zip(record['session_ids'], record['observed_indices'])):
                    ...
                    pred = geometric_predictions(p, record['scenario'], rn)
                    # A mechanism-aware attacker removes the publicly known fixed tracks.
                    adaptive_view = {'events': [{**e, 'candidates': e['candidates'][2:]} for e in p['events']]}
                    pred.update({'adaptive_only_'+name: value for name, value in geometric_predictions(adaptive_view, record['scenario'], rn).items()})
    ...
    result = {'schema': 'public-backbone-case-development-v1',
        'scope': 'public fixed2 plus adaptive3 residual cover, total K5/L10; same private anchors; core development; finite geometry can remove fixed tracks; new matched learned attacks required',
        ...
        'code_sha256': sha(Path(__file__)), 'source_sha256': {p: sha(ROOT/p) for p in (
            ...
            'benchmark/engines/slack_progress.py', 'benchmark/engines/public_backbone.py', 'evaluation/public_cover.py',
