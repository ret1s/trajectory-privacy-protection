"""Development ablation: planar Laplace vs REM with the same service planner."""
...
OUT = ROOT/'artifacts/benchmarks/research_loop/iteration26_planar_anchor_cases.json'
...
def main():
    ...
    belief = PlanarAnchorModel(ResponseAwareAnchorModel(base, deeper))
    factories = {'planar_paced': partial(PlanarPacedLaneDummy, utility_slack=0.),
                 'planar_paced_slack03': partial(PlanarPacedLaneDummy, utility_slack=.03)}
    models, setup = {}, {}
    ...
    for ordinal, control in enumerate(clocks):
        ...
        for method, model in models.items():
            ...
            assert coupled_ledger is None or model.evaluator_ledger == coupled_ledger
            coupled_ledger = model.evaluator_ledger
            ...
            executions.append({'session_id': sid, 'family_id': control['family_id'], 'split': control['split'],
                ...
                'evaluator_anchor_sha256': digest, 'evaluator_anchors': model.evaluator_anchors,
    ...
    result = {'schema': 'planar-anchor-case-development-v1',
        'scope': 'standard continuous planar-Laplace/private-reuse primitive with original reachable planner; total K5/L10; same ideal .23 cap, different anchors and branch ledger from REM; core development; matched learned attacks not yet run',
        'runtime_scope': 'descriptive only; iteration25 may run concurrently; no comparative speed inference',
        'control_source_sha256': sha(SOURCE), 'belief_model_sha256': belief.sha256, 'dataset_sha256': sha(DATA), 'resources': metadata,
        ...
        'code_sha256': sha(Path(__file__)), 'source_sha256': {p: sha(ROOT/p) for p in (
            ...
            'benchmark/engines/slack_progress.py', 'benchmark/engines/planar_paced.py',
            'benchmark/planar_anchor.py', 'benchmark/anchor_belief.py',
