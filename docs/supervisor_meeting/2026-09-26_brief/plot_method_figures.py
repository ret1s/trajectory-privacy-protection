...
box(.29,.43,.18,.17,'5. Cổng biên','S9/S10: stream v1\nđã kiểm tra ghép lõi',True)
...
ax.plot([.47,.50,.76],[.46,.35,.35],color=ink,lw=1.2);arr((.76,.35),(.79,.46));ax.text(.61,.36,'phát trễ qua buffer',ha='center',fontsize=8,color=ink);arr((.875,.42),(.70,.27));ax.text(.87,.405,'phản hồi POI',ha='center',fontsize=8,color=ink,bbox={'facecolor':'white','edgecolor':'none','pad':1})
...
ax.text(.04,.015,'Bước 5: có lớp stream; cache/padding và benchmark endpoint đầy đủ còn thiếu.',fontsize=9,color=gray)
