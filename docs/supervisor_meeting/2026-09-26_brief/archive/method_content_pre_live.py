...
def add_method_content(ns):
    ...
    sources=['artifacts/benchmarks/paper_benchmark/results.json','artifacts/benchmarks/fresh_switching/confirmation.json',
      ...
      'artifacts/benchmarks/report_boundary_audit/results.json','artifacts/benchmarks/contribution_stress/results.json','experiments/contribution_stress.py','docs/research/contribution_stress_protocol.md','benchmark/engines/budgeted.py',
      'benchmark/engines/contextual_lane.py','benchmark/engines/service_cover.py','benchmark/engines/fair_cover.py',
      'benchmark/engines/switching_cover.py','benchmark/switching_belief.py','core/mechanisms.py','core/boundary_release.py','artifacts/benchmarks/boundary_schedule/results.json']
    evidence={'status':'historical_measured_results_plus_new_exploratory_reanalysis','checked_on':'2026-09-24',
    ...
    figure('architecture','Hình 1. Kiến trúc lõi đã có trong mã nguồn và vị trí lớp stream bảo vệ biên S9/S10. Neo và nhãn thật không thuộc transcript máy chủ.')
    table(['Khối','Đầu vào → đầu ra','Trạng thái'],[
      ...
      ['Cổng bảo vệ biên','Transcript đã bảo vệ + chính sách biên → lịch công bố có kiểm soát.','Đã có lớp stream và kiểm tra ghép lõi; chưa có benchmark privacy/POI endpoint đầy đủ.']],[3.6,7.5,5.8])
    ...
    from endpoint_extension import method
    method(ns)
    page();sub('Vì sao nhiễu từng vị trí chưa đủ?')
    ...
    p('Trạng thái hiện tại: lõi BR đã hiện thực; cắt thêm cửa sổ đã chạy được dưới dạng audit ngoại tuyến; buffer nhân quả đã có trong BR-Boundary v1; cache, padding và bảo vệ liên kết phiên chưa được tích hợp/đo đầu-cuối. Kiểm tra code/lịch phát không thay thế benchmark privacy–utility của cơ chế kết hợp.')

    from contribution_content import add
    add(ns)
