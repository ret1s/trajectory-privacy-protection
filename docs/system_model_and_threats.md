...
| **S2** | **Map-matching / lọc off-road** (Takagi; RAoPT; Strava snap-to-street) | Biết lưới đường | Chuỗi điểm công bố | **On-road theo cấu trúc** (REM/T-REM output là đỉnh đồ thị) → không còn bề mặt map-matching để loại. Đo bằng `on_road_rate` = khoảng cách point-to-**edge** (đã sửa R4-003; trước đây đo tới đỉnh nên phóng đại off-road): REM/T-REM 1,00, PL ~0,82–0,92 (off-edge chỉ ~8–18%, không phải 41–54%). Đây là tính chất **cấu trúc**, chưa phải bằng chứng kháng RAoPT (cần chạy attack trực tiếp). |
