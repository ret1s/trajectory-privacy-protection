...
`evaluation/`. Mục 7 là **khuyến nghị bước tiếp theo** (bám
[`backlog.md`](backlog.md)). Mục 8 là các điểm chưa
chắc chắn.
...
| **S4** | **Averaging / báo cáo lặp từ điểm tĩnh** (Andrés nε; Mendes PoPETs'20; EPZ Strava 84–95%; fingerprint nhà của broker/NYT) | Thu nhiều release của cùng điểm thật theo thời gian | release lặp từ một chỗ | **Đừng phát nhiễu độc lập mới cho cùng điểm thật**: memoization / nhiễu tương quan (dự báo) + **kế toán ε·T mức quỹ đạo**. *Trạng thái:* **SM-REM** memoize static exact-repeat (cache ⇒ averaging vô hiệu); **PR-SM-REM** dùng noisy-threshold reuse test cho per-release `(ε_test+ε_release)`-bound (thay ratio ∞ của SM-REM). *Còn hở thành thật:* w-event budget manager thật + pattern thăm-lại (revisit) chưa được che — T-REM chỉ ràng buộc successor. Ghi làm future work; báo cáo ε_total. |
...
## 7. Khuyến nghị bước tiếp theo (bám `backlog.md`)
...
1. **Viết chương "Threat Model & Evaluation Scenarios" từ taxonomy S1–S8.** Đây là đóng góp "grounded
   ...
   + metric tương ứng. S1/S2/S3/S5 đã có code; chỉ cần viết + dẫn benchmark trong
   `datasets_metrics_related_work.md`.
   → phục vụ TODO "attack methods" và "dataset → metrics → algorithms → attack methods".

2. **Bịt khe hở S4 (averaging/retry) — đúng TODO "note retry mechanism" trong `backlog.md`.** Đã làm:
   (a) **memoization** (SM-REM cache release theo public cell để thăm lại cùng chỗ trả cùng fake) +
   **predictive/noisy-threshold reuse** (PR-SM-REM, kiểu Chatzikokolakis PETS'14); (b) thí nghiệm
   "báo cáo N lần từ một điểm nhà tĩnh → sai số adversary theo N" (`run_averaging_multi`) so 6 cơ chế
   với hai estimator (mean + MLE proxy). Còn lại (future work): **w-event budget manager thật** và
   che revisit-pattern. Nêu Andrés (nε) + Mendes PoPETs'20 làm attack neutralize. Đây là điểm phản biện reviewer
