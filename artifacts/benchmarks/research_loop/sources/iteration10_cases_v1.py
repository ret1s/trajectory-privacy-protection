...
OUT = ROOT/'artifacts/benchmarks/research_loop/iteration10_cases.json'
METHODS = {'raw': None, 'switching_core': SwitchingQuotientCoverLaneDummy,
           'filter_progress': MatchedFilteredProgressCoverLaneDummy,
           'filter_switching_progress': MatchedSwitchingProgressCoverLaneDummy}
...
def recall_pair(context, deeper, states, truth):
    ...
    return {str(L): float(np.mean(v)) for L, v in scores.items()}
    ...
    for method in METHODS:
        for case in sorted({r['case_id'] for r in rows}):
            ...
            summaries.append({'method': method, 'case_id': case, 'status': 'evaluated_finite_attack_bank',
                ...
                'hits': hits,
                'recall': {L: float(np.mean([r['recall'][L] for r in val])) for L in ('5', '10')},
                'pass_90pct_case_recall_L10': bool(np.mean([r['recall']['10'] for r in val]) >= .9)})
    return summaries
...
def main():
    ...
    models = {name: cls(rn, belief_model=belief, k=5, budget=.24, horizon=12,
                        rng=np.random.default_rng(0)) for name, cls in METHODS.items() if cls}
    executions = []
    lookup = {}
        ...
        for rep in range(2):
            ...
            for method, cls in METHODS.items():
                ...
                execution = {'session_id': sid, 'family_id': session_info[sid][0], 'split': session_info[sid][1],
                             ...
                             'whole_session_recall': {L: float(np.mean([u[L] for u in utilities.values()])) for L in ('5', '10')},
                             'step_ms': list(model.step_ms) if model else [],
                             'evaluator_ledger': model.evaluator_ledger if model and method.startswith('filter') else None}
                executions.append(execution)
                lookup[sid, rep, method] = execution
        print(sid, len(clock), 'queries; completed 8 method/repetition runs', flush=True)
    rows = []
    for r in records:
        for rep in range(2):
            for method in METHODS:
                ...
                rows.append({'record_id': r['record_id'], 'case_id': r['case_id'], 'scenario': r['scenario'],
                             ...
                             'recall': {L: float(np.mean([u[L] for u in utility])) for L in ('5', '10')}})
    result = {'schema': 'persistent-exact-case-development-v1',
              ...
              'source_sha256': {p: sha(ROOT/p) for p in (
                  ...
                  'benchmark/engines/switching_progress.py', 'benchmark/switching_belief.py',
